...
class WYSIHtml5Plugin(BaseAdminPlugin):
    ...
    def get_media(self, media):
        if self.include_html5:
            media.add_js([self.static('xcms/js/wysihtml5-0.3.0.min.js'), 
                self.static('xcms/js/bootstrap-wysihtml5.js'),
                self.static('xcms/js/locales/bootstrap-wysihtml5.zh-CN.js'),
                self.static('xcms/js/form_wysi.js')])
            media.add_css({'screen': [self.static('xcms/css/wysiwyg-color.css'), self.static('xcms/css/bootstrap-wysihtml5.css')]})
        if self.include_tinymce:
            media.add_js([
                self.static('xcms/tiny_mce/jquery.tinymce.js'),
                self.static('xcms/js/form_wysi.js')])
